  ...
  return (
    <>
      <Checkbox
        ...
        value=""
        checked={selected.length === item.length}
        indeterminate={selected.length > 0 && selected.length !== item.length}
      >
        Zaznacz wszystko
      </Checkbox>
      <br />
      <Checkbox.Group value={selected} onChange={handleChange} style={{ width: '100%' }}>
        <List
          dataSource={item}
          ...
          footer={
            <Space.Compact style={{ width: '100%', marginTop: '8px' }}>
              <Input
                placeholder={defaultOptions[0]}
                value={newItem}
                onChange={handleIngredientChange}
                onKeyDown={handleKeyDown}
              />
              <Button type="default" onClick={handleAddOption}>
                Dodaj
              </Button>
            </Space.Compact>
          }
